...
/// struct for typed errors of method [`logged_server_data_bfv_marne_serverarray_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LoggedServerDataBfvMarneServerarrayGetError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`print_logged_data_bfv_marne_statusarray_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PrintLoggedDataBfvMarneStatusarrayGetError {
    Status404(),
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`status_bfv_marne_status_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StatusBfvMarneStatusGetError {
    Status404(),
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}
...
pub async fn logged_server_data_bfv_marne_serverarray_get(configuration: &configuration::Configuration, days: Option<i32>, serverid: Option<&str>, servername: Option<&str>) -> Result<serde_json::Value, Error<LoggedServerDataBfvMarneServerarrayGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_days = days;
    let p_serverid = serverid;
    let p_servername = servername;

    let uri_str = format!("{}/bfv_marne/serverarray/", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<LoggedServerDataBfvMarneServerarrayGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn print_logged_data_bfv_marne_statusarray_get(configuration: &configuration::Configuration, days: Option<i32>, region: Option<models::FrostbiteRegionsWithMultiple>, platform: Option<models::FrostbitePlatforms>, r#type: Option<models::StatusArrayType>) -> Result<models::FrostbiteStatusArray, Error<PrintLoggedDataBfvMarneStatusarrayGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_days = days;
    let p_region = region;
    ...
    let uri_str = format!("{}/bfv_marne/statusarray/", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<PrintLoggedDataBfvMarneStatusarrayGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// For platform there is pc, xboxone and ps4
pub async fn status_bfv_marne_status_get(configuration: &configuration::Configuration, platform: Option<&str>) -> Result<models::FrostbiteGameStatus, Error<StatusBfvMarneStatusGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_platform = platform;

    let uri_str = format!("{}/bfv_marne/status/", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<StatusBfvMarneStatusGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
